...
public class SettingsActivity extends AbstractSettingsActivity {
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        findPreference(GBPrefs.AUTO_EXPORT_ENABLED).setOnPreferenceChangeListener((preference, autoExportEnabled) -> {
            int autoExportInterval1 = GBApplication.getPrefs().getInt(GBPrefs.AUTO_EXPORT_INTERVAL, 0);
            PeriodicExporter.scheduleAlarm(getApplicationContext(), autoExportInterval1, (boolean) autoExportEnabled);
            return true;
        });
        ...
        pref.setOnPreferenceChangeListener((preference, autoFetchInterval) -> {
            ...
            preference.setSummary(summary1);
            return true;
        });
        ...
        theme.setOnPreferenceChangeListener((preference, newVal) -> {
        ...
        try {
            ...
            try {
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    int cursorColumnIndex = cursor.getColumnIndex(DocumentsContract.Document.COLUMN_DISPLAY_NAME);
                    if (cursorColumnIndex >= 0) {
                        return cursor.getString(cursorColumnIndex);
                    }
                }
